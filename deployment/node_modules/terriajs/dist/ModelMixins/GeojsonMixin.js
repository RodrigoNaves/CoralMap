...
export default function GeoJsonMixin(Base) {
    class GeoJsonMixin extends DiscretelyTimeVaryingMixin(MappableMixin(UrlMixin(Base))) {
        ...
        loadGeoJsonDataSource(geoJson) {
            ...
            return makeRealPromise(GeoJsonDataSource.load(geoJson, options)).then(dataSource => {
                ...
                for (let i = 0; i < entities.values.length; ++i) {
                    ...
                    if (isDefined(entity.billboard) && isDefined(options.markerUrl)) {
                        ...
                        entity.point = new PointGraphics({
                            color: new ConstantProperty(getColor(defaultValue(properties && ((_c = properties["marker-color"]) === null || _c === void 0 ? void 0 : _c.getValue()), options.markerColor))),
                            pixelSize: new ConstantProperty(defaultValue(parseMarkerSize(properties && ((_d = properties["marker-size"]) === null || _d === void 0 ? void 0 : _d.getValue())), options.markerSize / 2)),
                            outlineWidth: new ConstantProperty(defaultValue(properties && ((_e = properties["stroke-width"]) === null || _e === void 0 ? void 0 : _e.getValue()), options.strokeWidth)),
                            outlineColor: new ConstantProperty(getColor(defaultValue(properties && ((_f = properties.stroke) === null || _f === void 0 ? void 0 : _f.getValue()), options.polygonStroke))),
                            heightReference: new ConstantProperty(options.clampToGround
...
function reprojectToGeographic(geoJson, proj4ServiceBaseUrl) {
    let code;
    ...
    return makeRealPromise(Reproject.checkProjection(proj4ServiceBaseUrl, code)).then(function (result) {
        if (result) {
