...
export default class TableStyle {
    ...
    get timeIntervals() {
        ...
        const intervals = timeColumn.valuesAsJulianDates.values.map((date, i) => {
            ...
            const startDate = (_b = (_a = this.startJulianDates) === null || _a === void 0 ? void 0 : _a[i]) !== null && _b !== void 0 ? _b : date;
            const finishDate = (_d = (_c = this.finishJulianDates) === null || _c === void 0 ? void 0 : _c[i]) !== null && _d !== void 0 ? _d : undefined;
    ...
    get startJulianDates() {
        ...
        this.rowGroups.forEach(([groupId, rowIds]) => {
            ...
            const firstRowId = rowIds
                ...
                .sort((idA, idB) => JulianDate.compare(startDates[idA], startDates[idB]))[0];
            // Set it to earliest date in the entire column
            if (isDefined(firstRowId))
                startDates[firstRowId] = firstDate;
        });
        return startDates;
    }
    /**
    ...
    get finishJulianDates() {
        ...
        if (this.timeTraits.displayDuration !== undefined) {
            return startDates.map(date => date
                ? JulianDate.addMinutes(date, this.timeTraits.displayDuration, new JulianDate())
                : null);
        }
        const finishDates = [];
     ...
     */
    get rowGroups() {
        const groupByCols = this.idColumns ||
            filterOutUndefined([this.latitudeColumn, this.longitudeColumn]);
        const tableRowIds = this.tableModel.rowIds;
        return Object.entries(groupBy(tableRowIds, rowId => groupByCols.map(col => col.values[rowId]).join("-")));
    }
    /**
    ...
    calculateFinishDatesFromStartDates(startDates, defaultFinalDurationSeconds) {
        ...
        const lastDate = (_a = this.timeColumn) === null || _a === void 0 ? void 0 : _a.valuesAsJulianDates.maximum;
        return startDates.map(date => {
            ...
            if (nextDate) {
                ...
                const finalDate = addSecondsToDate(sortedStartDates[sortedStartDates.length - 1], finalDurationSeconds);
                return finalDate;
            }
        });
    }
