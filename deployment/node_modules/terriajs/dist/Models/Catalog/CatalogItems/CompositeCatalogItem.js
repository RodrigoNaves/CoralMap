...
export default class CompositeCatalogItem extends MappableMixin(CatalogMemberMixin(CreateModel(CompositeCatalogItemTraits))) {
    ...
    async forceLoadMetadata() {
        Result.combine(await Promise.all(this.memberModels
            .filter(CatalogMemberMixin.isMixedInto)
            .map(async (model) => await model.loadMetadata())), "Failed to load composite catalog items metadata").throwIfError();
    }
    async forceLoadMapItems() {
        Result.combine(await Promise.all(this.memberModels
            .filter(MappableMixin.isMixedInto)
            .map(model => model.loadMapItems())), "Failed to load composite catalog items mapItems").throwIfError();
    }
