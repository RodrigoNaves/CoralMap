...
export class SdmxServerStratum extends LoadableStratum(SdmxCatalogGroupTraits) {
    ...
    createMemberFromLayer(node) {
        ...
        if (node.members && Object.keys(node.members).length > 0) {
            ...
            groupModel.setTrait(CommonStrata.underride, "name", node.item.name || node.item.id);
            groupModel.setTrait(CommonStrata.underride, "members", filterOutUndefined(Object.values(node.members).map(member => this.getMemberId(member))));
