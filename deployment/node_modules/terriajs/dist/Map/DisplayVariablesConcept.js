...
var DisplayVariablesConcept = function (name, options) {
    const that = this;
    name = defaultValue(name, i18next.t("map.displayVariablesConcept.defaultName"));
...
DisplayVariablesConcept.prototype.toggleActiveItem = function (concept) {
    ...
    if (isActive &&
        ...
        this.items.every(child => child === concept ? child.isActive : !child.isActive)) {
        return;
    }
    // If we are activating an item, but only allow one selection, we need to deselect the others.
    if ((!isActive && !this.allowMultiple) ||
    ...
    if (defined(this.toggleActiveCallback &&
...
//# sourceMappingURL=DisplayVariablesConcept.js.map