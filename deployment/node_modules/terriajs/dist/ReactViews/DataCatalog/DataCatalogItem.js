...
// Individual dataset
export const DataCatalogItem = observer(createReactClass({
    ...
    onBtnClicked(event) {
        runInAction(() => {
            ...
            if (CatalogFunctionMixin.isMixedInto(this.props.item) ||
    ...
    async toggleEnable(event) {
        ...
        try {
            ...
            if (this.props.terria.workbench.contains(this.props.item) &&
                !keepCatalogOpen) {
                this.props.viewState.closeCatalog();
                (_a = this.props.terria.analytics) === null || _a === void 0 ? void 0 : _a.logEvent(Category.dataSource, toAdd
                    ? DataSourceAction.addFromCatalogue
                    : DataSourceAction.removeFromCatalogue, getPath(this.props.item));
            }
        }
        catch (e) {
    ...
    render() {
        ...
        return (React.createElement(CatalogItem, { onTextClick: this.setPreviewedItem, selected: this.isSelected(), text: item.nameInCatalog, isPrivate: item.isPrivate, title: getPath(item, " -> "), btnState: this.getState(), onBtnClick: this.onBtnClicked, 
            // All things are "removable" - meaning add and remove from workbench,
            //    but only user data is "trashable"
            trashable: this.props.removable, onTrashClick: this.props.removable
                ? () => {
                    this.onTrashClicked();
                }
                : undefined, titleOverrides: STATE_TO_TITLE }));
    },
    getState() {
