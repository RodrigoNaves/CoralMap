...
let Chart = class Chart extends React.Component {
    ...
    render() {
        ...
        return (React.createElement(ZoomX, { surface: "#zoomSurface", initialScale: this.initialXScale, scaleExtent: [1, Infinity], translateExtent: [
            ...
            React.createElement(Legends, { chartItems: this.chartItems.length > 4 ? [] : this.chartItems }),
        ...
        return chartItems.map((chartItem, i) => {
            switch (chartItem.type) {
                ...
                case "momentPoints": {
                    ...
                    const basisItemIndex = chartItems.findIndex(item => item.type === "line" && item.xAxis.scale === "time");
                    return (React.createElement(MomentPointsChart, { key: chartItem.key, ref: this.chartRefs[i], id: sanitizeIdString(chartItem.key), chartItem: chartItem, scales: initialScales[i], basisItem: chartItems[basisItemIndex], basisItemScales: initialScales[basisItemIndex] }));
                }
                case "momentLines": {
                ...
                case "lineAndPoint": {
                    return (React.createElement(LineAndPointChart, { key: chartItem.key, ref: this.chartRefs[i], id: sanitizeIdString(chartItem.key), chartItem: chartItem, scales: initialScales[i] }));
                }
            }
        });
    }
