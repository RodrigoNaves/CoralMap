    ...
    render() {
        ...
        return (React.createElement("div", { className: Styles.root },
            React.createElement("div", { className: Styles.title },
                concept.name,
                ":"),
            React.createElement(For, { each: "group", index: "i", of: activeLeafNodesByParent },
                React.createElement(ActiveConcept, { key: i, rootConcept: concept, activeLeafNodesWithParent: group, isLoading: isLoading })),
            React.createElement(If, { condition: activeLeafNodesByParent.length === 0 &&
                    openDescendantsWithoutActiveChildren.length === 0 },
                React.createElement("div", { className: Styles.noConditions }, "None")),
            React.createElement(If, { condition: openDescendantsWithoutActiveChildren.length > 0 && !isLoading },
                React.createElement(OpenInactiveConcept, { rootConcept: concept, openInactiveConcept: openDescendantsWithoutActiveChildren[0] })),
            React.createElement(If, { condition: concept.allowMultiple &&
                    openDescendantsWithoutActiveChildren.length === 0 },
                React.createElement(AddButton, { rootConcept: concept, numberOfExisting: activeLeafNodesByParent.length, t: t }))));
    }
});
/**
...
const AddButton = createReactClass({
    ...
    render() {
        const { t } = this.props;
        const addText = this.props.numberOfExisting > 0
            ? t("concept.summary.addMoreText")
            : t("concept.summary.addFirstText");
        return (React.createElement("div", { className: Styles.section },
            React.createElement("button", { onClick: this.addNew, className: Styles.btnAddNew },
                React.createElement(Icon, { glyph: Icon.GLYPHS.add }),
                React.createElement("span", { className: Styles.text }, addText))));
    }
});
module.exports = withTranslation()(SummaryConcept);
//# sourceMappingURL=SummaryConcept.js.map